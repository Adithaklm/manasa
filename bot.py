...
# Local Bot API Server endpoints.
# Example:
# TELEGRAM_BASE_URL=http://LOCAL_API_HOST:8081/bot
# TELEGRAM_FILE_BASE_URL=http://LOCAL_API_HOST:8081/file/bot
    ...
    if action == "codec":
        ...
        await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(v, callback_data=f"set_preset:{v}") for v in vals[i:i+3]]
...
def build_app():
    if bool(BASE_URL) != bool(FILE_BASE_URL):
        raise RuntimeError(
            "Set both TELEGRAM_BASE_URL and TELEGRAM_FILE_BASE_URL for Local Bot API, "
            "or leave both unset to use the official Telegram Bot API."
        )
...
if __name__ == "__main__":
    ...
    if not BASE_URL:
        logging.warning(
            "Using the official Telegram Bot API. Large files require a Local Bot API Server."
        )
